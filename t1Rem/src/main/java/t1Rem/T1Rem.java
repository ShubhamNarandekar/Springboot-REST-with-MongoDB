...
public class T1Rem {
    ...
    public Grade generateGrade(StudentInfo info) {
        ...
        if(aptitude > 3.7) { percentage = 75+(double) random.nextInt(25);}
        else if(aptitude > 3.0) { percentage = 50+(double) random.nextInt(50);}
        else if(aptitude > 2.5) {percentage = 25+(double) random.nextInt(75);}
        else {percentage = random.nextInt(100);}
